...
class NexusStudioApp(ctk.CTk):
    ...
    def show_login_screen(self):
        # FIX: Use place() to set dimensions instead of constructor
        ...
        title = ctk.CTkLabel(
            self.login_frame, 
            text="🛡️ Nexus Multi-Tier Network", 
        ...
        subtitle = ctk.CTkLabel(
            self.login_frame, 
            text="Device Verification & Secure Settlement Gateway", 
        ...
        lbl1 = ctk.CTkLabel(
            self.login_frame, 
            text="ENTER NAME / BUSINESS EMAIL:", 
        ...
        lbl_sso = ctk.CTkLabel(
            self.login_frame, 
            text="ONE-TAP SECURE CLOUD AUTHENTICATION:", 
            font=ctk.CTkFont(size=11, weight="bold"), 
            text_color="#a1a1aa"
        )
        ...
        self.google_btn = ctk.CTkButton(
            self.login_frame, 
            text="🔴 Authenticate with Google Live", 
            font=ctk.CTkFont(weight="bold"), 
            fg_color="#27272a", 
            hover_color="#3f3f46", 
            height=42, 
            command=lambda: self.process_sso("Google")
        )
        self.google_btn.pack(fill="x", padx=40, pady=5)

        self.ms_btn = ctk.CTkButton(
            self.login_frame, 
            text="🔵 Authenticate with Microsoft Live", 
            font=ctk.CTkFont(weight="bold"), 
            fg_color="#27272a", 
            hover_color="#3f3f46", 
            height=42, 
            command=lambda: self.process_sso("Microsoft")
        )
        self.ms_btn.pack(fill="x", padx=40, pady=5)

        self.login_btn = ctk.CTkButton(
            self.login_frame, 
            text="Access Secure Workspace", 
            font=ctk.CTkFont(weight="bold"), 
            fg_color="#4f46e5", 
            hover_color="#4338ca", 
            height=45, 
            command=lambda: self.process_sso("Manual")
        )
    ...
    def build_main_workspace(self):
        ...
        self.new_chat_btn = ctk.CTkButton(
            ...
            border_width=1, 
            height=38, 
            command=self.clear_and_new_chat
        )
        ...
        u_lbl = ctk.CTkLabel(
            p_box, 
            text=f"User: {db['current_user']}", 
            font=ctk.CTkFont(size=12, weight="bold"), 
            text_color="#e4e4e7"
        )
        u_lbl.pack(anchor="w", padx=12, pady=(8, 2))

        status_color = "#10b981" if ("Developer" in db["user_role"] or "Verified" in db["user_role"]) else "#f59e0b"
        r_lbl = ctk.CTkLabel(
            p_box, 
            text=f"🛡️ Status: {db['user_role']}", 
        ...
        if "Developer" in db["user_role"]:
            ...
            e_title = ctk.CTkLabel(
                earn_box, 
                text="💰 TOTAL REAL-TIME EARNINGS", 
                font=ctk.CTkFont(size=11, weight="bold"), 
                text_color="#34d399"
            )
            e_title.pack(anchor="w", padx=12, pady=(8, 2))

            total_money = sum(data.get("amount", 0) for data in db["active_subscribers"].values())

            e_stat = ctk.CTkLabel(
                earn_box, 
                text=f"Collected: ₹{total_money} (SBI)", 
        ...
        self.input_bar = ctk.CTkEntry(
            cmd_frame, 
            placeholder_text="Ask Nexus AI to chat, draw images, write songs or process cinematic video frames...", 
            height=46, 
            fg_color="#141416", 
            border_color="#232329", 
            corner_radius=25
        ...
        self.send_btn = ctk.CTkButton(
            ...
            height=44, 
            corner_radius=22, 
            command=self.route_intelligent_prompt
        )
        ...
        if "video" in p_lower or "render" in p_lower or "cinematic" in p_lower:
            if "Developer" not in db["user_role"] and "Verified" not in db["user_role"]:
                ...
                self.update_chat_display(
                    "🤖 NEXUS AI (ACCESS GRANTED):\n⚡ Heavy-computation cloud render initiated. "
                    "Video sequences successfully generated and appended to storage nodes.\n\n" + "─"*50 + "\n\n"
                )
            ...
            if img_asset:
                ...
                self.update_chat_display(
                    "❌ Engine Security/Quota Error: Unable to complete image rendering. "
                    "Ensure your environment token is valid.\n\n" + "─"*50 + "\n\n"
                )
            return
            ...
            if "Engine Error" in lyrics or "Error" in lyrics:
            ...
            try:
                ...
                if sys.platform == "win32":
                    ...
                    os.system(f"open {track_path}")
                else:
                    os.system(f"xdg-open {track_path}")
                ...
                self.update_chat_display(f"❌ Audio Core Error: {str(e)}\n\n" + "─"*50 + "\n\n")
            return
    ...
    def show_subscription_paywall(self):
        ...
        lbl_top = ctk.CTkLabel(
            paywall, 
            text="SELECT YOUR WORKSPACE LEVEL", 
        ...
        lbl_sub = ctk.CTkLabel(
            paywall, 
            text="Unlock advanced video rendering models matching your content production scaling goals.", 
            font=ctk.CTkFont(size=11), 
            text_color="#a1a1aa"
        )
        ...
        ctk.CTkLabel(
            card1, 
            text="⚡ Nexus Standard (Flexible Plan)", 
            font=ctk.CTkFont(size=14, weight="bold"), 
            text_color="#60a5fa"
        ).pack(pady=(15, 5))
        ...
        ctk.CTkLabel(
            card1, 
            text="Best optimized for casual & regular content creators.\n\n✔ 30 High-Speed Video Renders / Month\n✔ Standard Cloud Node Speed\n✔ Direct GPay Transaction Settlement", 
            font=ctk.CTkFont(size=11), 
            text_color="#a1a1aa", 
            justify="left"
        ).pack(pady=10)
        ...
        ctk.CTkLabel(
            card2, 
            text="Tailored for premium multi-modal generation workloads.\n\n✔ Unlimited Heavy Cinematic Video Renders\n✔ Priority Alpha GPU Dedicated Pipelines\n✔ Instant Activation and Lifetime Support", 
            font=ctk.CTkFont(size=11), 
            text_color="#a1a1aa", 
            justify="left"
        ).pack(pady=10)

        bottom_frame = ctk.CTkFrame(paywall, fg_color="#1c1c1f", corner_radius=10)
        bottom_frame.pack(fill="x", padx=40, pady=25)

        upi_lbl = ctk.CTkLabel(
            bottom_frame, 
            text=f"Official Settlement Gateway ID: {db['developer_upi']}", 
            font=ctk.CTkFont(size=12, weight="bold"), 
            text_color="#3b82f6"
        )
        upi_lbl.pack(pady=8)
        ...
        inputs_sub.pack(fill="x", padx=20, pady=10)

        self.chosen_tier = ctk.CTkOptionMenu(
            inputs_sub, 
            values=["Nexus Standard (₹399)", "Nexus Elite (₹1000)"], 
            fg_color="#27272a", 
            button_color="#3f3f46"
        )
        self.chosen_tier.pack(side="left", padx=5)

        self.txn_entry = ctk.CTkEntry(
            inputs_sub, 
            placeholder_text="Enter 12-Digit GPay Ref / UTR Code", 
            width=250
        )
        ...
        def verify_and_grant():
            ...
            paywall.destroy()
            self.update_chat_display(
                f"⚡ DISPATCH UPDATE: Reference number {tx} received for {tier_name}. "
                "Access configurations applied successfully!\n\n" + "─"*50 + "\n\n"
            )
    ...
    def clear_and_new_chat(self):
        ...
        self.display_box.insert(
            "end", 
            "✨ SYSTEM: New isolated chat workflow sequence successfully active.\n\n" + "─"*50 + "\n\n"
        )
        self.display_box.configure(state="disabled")
    ...
    def render_chat_history_from_db(self):
        ...
        self.display_box.insert(
            "end", 
            f"✨ SYSTEM: Welcome back! Previous database logs successfully reloaded.\n\n" + "─"*50 + "\n\n"
        )
    ...
    def trigger_signout(self):
        ...
        os.execl(python_executable, python_executable, __file__)
